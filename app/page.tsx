...
function HomeContent() {
  ...
  const activeFilters: { label: string; onRemove: () => void }[] = [
    ...selectedBrands.map((b) => ({ label: `Marca: ${b}`, onRemove: () => toggleBrand(b) })),
    ...(selectedModel ? [{ label: `Modelo: ${selectedModel}`, onRemove: () => setSelectedModel('') }] : []),
    ...selectedCategories.map((c) => ({ label: `Cat: ${c}`, onRemove: () => toggleCategory(c) })),
  ...
  const filteredProducts = mockProducts
    .filter((p) => {
      ...
      const matchesCat = !selectedCategories.length || selectedCategories.includes(p.category);
      const matchesCond = !selectedConditions.length || selectedConditions.includes(p.condition);
      return matchesSearch && matchesBrand && matchesModel && matchesCat && matchesCond;
    })
    .sort((a, b) => {
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {brands.map((brand) => (
            <label key={brand} className="flex items-center gap-2 text-sm cursor-pointer hover:text-gray-900">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {categories.map((cat) => (
            <label key={cat} className="flex items-center gap-2 text-sm cursor-pointer hover:text-gray-900">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {conditions.map((cond) => (
            <label key={cond} className="flex items-center gap-2 text-sm cursor-pointer hover:text-gray-900">
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 lg:py-8">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {activeFilters.map((f, i) => (
              <span key={i} className="inline-flex items-center gap-1 bg-red-50 border border-red-200 text-red-700 text-xs px-2.5 py-1 rounded-full">
                {f.label}
                <button onClick={f.onRemove} className="hover:text-red-900 ml-0.5">
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
          <div>
            <h3 className="text-xl sm:text-2xl font-semibold mb-1">Catálogo de Piezas</h3>
            <p className="text-sm text-gray-600">{filteredProducts.length} producto(s) encontrado(s)</p>
          </div>
          ...
          <div className="hidden lg:block">
            <div className="bg-white rounded-lg border border-gray-200 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                {activeFilters.length > 0 && (
                  <button onClick={clearFilters} className="text-xs text-gray-500 hover:text-gray-800 flex items-center gap-1">
                    <RotateCcw className="h-3 w-3" />
                    Limpiar todo
                  </button>
                )}
          ...
          <div className="lg:col-span-3">
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
                ...
                <button onClick={clearFilters} className="text-red-600 hover:underline text-sm font-medium">
                ...
                {filteredProducts.map((product) => (
                  <div key={product.id} onClick={() => router.push(`/products/${product.id}`)}
                    ...
                    <div className="p-3 sm:p-4">
                      ...
                      <h3 className="text-sm sm:text-base font-semibold mb-0.5 line-clamp-1">{product.name}</h3>
                      <p className="text-xs text-gray-500 mb-2">{product.brand} · {product.model}</p>
                      <p className="text-lg sm:text-xl font-bold text-red-600 mb-2">{formatPrice(product.price)}</p>
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        <span className="truncate max-w-[60%]">{product.seller}</span>
