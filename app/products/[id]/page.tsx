...
import {
  ArrowLeft, Heart, Share2, Package, Shield, Star, MapPin,
  ShoppingCart, Minus, Plus, ChevronLeft, ChevronRight,
  ZoomIn, X, Eye, AlertTriangle,
} from 'lucide-react';
...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  ...
  const detail = (mockProductDetails[id] ?? mockProductDetails['1']) as ProductDetail;
  ...
  const product = {
    ...
    condition: detail?.condition ?? base.condition,
    description: detail?.description ?? 'Sin descripción disponible.',
    images: detail?.images ?? [base.image],
    sellerInfo: detail?.sellerInfo ?? { name: base.seller, rating: 4.5, reviews: 12, verified: true },
    location: detail?.location ?? base.location,
    category: detail?.category ?? base.category,
    stock: detail?.stock ?? 1,
    specifications: (detail?.specifications ?? {}) as Record<string, string>,
    status: (base as Record<string, unknown>).status ?? 'active',
  };
  ...
  const prevImage = () => setActiveImg((i) => (i - 1 + product.images.length) % product.images.length);
          ...
          <div>
            <div className="flex items-start justify-between mb-3 sm:mb-4 gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm sm:text-base text-gray-600">{product.brand} • {product.model}</p>
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button className="h-9 w-9 sm:h-10 sm:w-10 border rounded-md hover:bg-gray-50 flex items-center justify-center" aria-label="Favoritos">
                  <Heart className="h-3 w-3 sm:h-4 sm:w-4" />
                </button>
                <button className="h-9 w-9 sm:h-10 sm:w-10 border rounded-md hover:bg-gray-50 flex items-center justify-center" aria-label="Compartir">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 sm:p-6 mb-4 sm:mb-6">
              <p className="text-2xl sm:text-3xl font-bold text-red-600 mb-2">{formatPrice(product.price)}</p>
              <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-gray-600">
                <div className="flex items-center gap-1">
                  <Package className="h-3 w-3 sm:h-4 sm:w-4" />
                  <span>{product.stock} disponibles</span>
                </div>
                <span className="px-2 py-1 bg-gray-100 rounded">{product.condition}</span>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-sm sm:text-base font-medium">{product.sellerInfo.name}</p>
                    {product.sellerInfo.verified && <Shield className="h-3 w-3 sm:h-4 sm:w-4 text-red-600" />}
                  </div>
            ...
            {user && !isSold ? (
              <div className="space-y-3 mb-4 sm:mb-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                  ...
                  <div className="flex items-center border rounded-lg">
                    ...
                    <span className="px-3 sm:px-4 py-2 min-w-[2.5rem] text-center text-sm">{quantity}</span>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4 sm:mb-6">
                <p className="text-sm text-yellow-800 mb-3 font-medium">
                  Inicia sesión para contactar al vendedor y comprar este producto
                </p>
                <Link href="/login"
            ...
            {Object.keys(product.specifications).length > 0 && (
              <div>
                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3">Especificaciones Técnicas</h3>
                <div className="bg-white rounded-lg shadow p-3 sm:p-4">
                  <dl className="space-y-2 sm:space-y-3">
                    {Object.entries(product.specifications).map(([key, value]: [string, string]) => (
                      <div key={key} className="flex flex-col sm:flex-row sm:justify-between py-2 border-b last:border-0 gap-1 sm:gap-0">
                        <dt className="text-xs sm:text-sm text-gray-600 font-medium">{key}</dt>
                        <dd className="text-xs sm:text-sm sm:text-right">{value}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
